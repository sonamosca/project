...
@section('title', 'Edit Programme') {{-- Changed Title --}}

@push('styles')
    {{-- Re-use the same styles as create.blade.php for consistency --}}
...
<div class="form-container">
    ...
    <div class="form-card">
        ...
        {{-- Edit Form --}}
        <form action="{{ route('admin.programmes.update', $programme->id) }}" method="POST"> {{-- Changed Route --}}
            @csrf
            @method('PUT') {{-- Specify PUT method --}}

            {{-- Programme Name Input --}}
            <div class="form-group">
                <label for="name">Programme Name:</label> {{-- Changed Label --}}
                <input type="text"
                       name="name"
                       id="name"
                       class="form-control @error('name') is-invalid @enderror"
                       value="{{ old('name', $programme->name) }}" {{-- Pre-fill with current programme name --}}
                       placeholder="e.g., Computer System and Network" {{-- Changed Placeholder --}}
                       required
                       autofocus>
                @error('name')
                    ...
                    @isset($departments)
                        @forelse($departments as $department)
                            <option value="{{ $department->id }}"
                                {{-- Pre-select based on old input OR current programme's department --}}
            ...
            <div class="form-actions">
                <a href="{{ route('admin.programmes.index') }}" class="btn btn-secondary"> {{-- Changed Route --}}
                    Cancel
                </a>
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save"></i> Update Programme {{-- Changed Button Text --}}
                </button>
            </div>
...
@push('scripts')
    {{-- No specific JS needed for this simple edit form --}}
@endpush